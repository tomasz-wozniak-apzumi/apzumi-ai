...
Your task, starting now: analyze the current changes, group them into focused logical commits, and commit each group following the Conventional Commits specification. Do not ask what to do, do not ask for confirmation, do not offer options — execute the steps below and finish with the summary in Step 6.

## Arguments

Arguments passed to this invocation: `$ARGUMENTS`

An empty value means no arguments — run with defaults: real commits (not `--dry`), branch creation enabled, ticket detected from the branch name.
...
If the working tree is clean (no staged, unstaged, or untracked changes), stop here and output only:
...
Otherwise, if the current branch is an integration branch (`main`, `master`, `develop`, `dev`, `release/*`), derive a suggested branch name from the dominant commit group and remember it for Step 5:

- **Ticket present** → `<type>/<TICKET>-<short-description>`
- **No ticket** → `<type>/<scope>-<short-description>` (lowercase, hyphenated)

Map the dominant Conventional Commits type to a branch prefix.
...
If `--no-branch` is off, the current branch is an integration branch, and it is not already a feature branch: create and switch to the suggested branch (from Step 1) before committing:
...
- **Header:**
  - `<type>/<name> (created)` — branch created this run (not `--dry`)
  - `Branch: <type>/<name> (suggested)` — `--dry` on an integration branch

...
2. **tasks.md is grounded and atomic** — one task = one logical commit, every
   ...
   platform's test code from its Regression scenarios — delegated to
   `@apzumi-generate-test-code`, and done when those tests *fail* for the right
   reason. That subagent is a **placeholder** awaiting an implementation; the
   workflow is shaped for it so filling it in needs no schema change.
3. **Two test layers that cannot be confused** — `tests.md` is **layer 1,
   ...
   tester-observable behaviour only), whose `Regression` rows become automated
   E2E tests. **Layer 2, unit & integration**, is hand-written in tasks.md from
   the specs and design. Task labels (`[E2E automation]`, `[Unit]`,
   `[Integration]`) keep them visibly distinct, and the validator rejects a
   layer-1 task in the layer-2 group. Manual QA never leaks into tasks.md.
...
│   │   └── apzumi-generate-test-code.md     (E2E generation — placeholder)
